...
class TTS(tts.TTS):
    ...
    def synthesize(
        ...
        return ChunkedStream(ssml, self._opts)
...
class ChunkedStream(tts.ChunkedStream):
    ...
    # azure-ssml version
    def __init__(self, ssml: str, opts: _TTSOptions) -> None:
        self._opts = opts
        self._ssml = ssml
        self._main_task: asyncio.Task | None = None
        self._queue = asyncio.Queue[Optional[tts.SynthesizedAudio]]()
    ...
    async def _run(self):
        try:
            ...
            def _synthesize() -> speechsdk.SpeechSynthesisResult:
                # return synthesizer.speak_text_async(self._text).get()  # type: ignore #non-azure-ssml version
                return synthesizer.speak_ssml_async(self._ssml).get()  # type: ignore #azure-ssml version
...
class _PushAudioOutputStreamCallback(speechsdk.audio.PushAudioOutputStreamCallback):
    ...
    def write(self, audio_buffer: memoryview) -> int:
        ...
        # audio = tts.SynthesizedAudio(text="", data=audio_frame) # non-azure-ssml version
        audio = tts.SynthesizedAudio(ssml="", data=audio_frame) # azure-ssml version
