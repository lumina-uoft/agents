...
class VoiceAssistant(utils.EventEmitter[EventTypes]):
    ...
    async def _synthesize_speech_co(
        ...
        # text: str, # non-azure-ssml version
        ssml: str, # azure-ssml version
        ...
        """
        # azure-ssml version
        data.collected_text += ssml
        tts_forwarder.push_text(ssml)
        tts_forwarder.mark_text_segment_end()
        ...
        try:
            async for audio in self._tts.synthesize(ssml): # azure-ssml version
            #async for audio in self._tts.synthesize(text): # non-azure-ssml version
                if first_frame:
                    first_frame = False
