...
class StreamAdapter(TTS):
    ...
    # azure-ssml version
    def synthesize(self, ssml: str) -> ChunkedStream:
        return self._tts.synthesize(ssml=ssml)
...
class StreamAdapterWrapper(SynthesizeStream):
    ...
    async def _synthesize(
        ...
        async with self._sem:
            # stream = self._tts.synthesize(text=sentence) # non-azure-ssml version
            stream = self._tts.synthesize(ssml=sentence) # azure-ssml version
            try:
